...
class SwitchTeamAnimation(fight: Fight, val futureAttacker: Fighter, val futureDefender: Fighter) :
    Animation(fight) {
    ...
    fun assignCreaturesToUI() {
        ...
        while (playerNumber <= 2) {
            ...
            while (creatureIndex < getPlayerByNumber(playerNumber).team.size) {
                ...
                getCreatureNameTextView(playerNumber, creatureIndex).text =
                    getPlayerByNumber(playerNumber).team[creatureIndex].creatureName
                getCreatureImageView(
                    playerNumber,
                    creatureIndex
                ).setImageResource(getPlayerByNumber(playerNumber).team[creatureIndex].imageResource)
